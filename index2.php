...
// Baby validation
$valid_babies = ["Alice", "Bob"];
$baby_condition = "";
if (isset($_POST['baby']) && $_POST['baby'] !== "all" && in_array($_POST['baby'], $valid_babies)) {
    $baby = $_POST['baby'];
    $baby_condition = "AND baby_name = ?";
}
...
// Bind parameters dynamically depending on filters
if ($type_condition && $baby_condition) {
    mysqli_stmt_bind_param($stmt, "iss", $days, $type, $baby);
} elseif ($type_condition) {
    mysqli_stmt_bind_param($stmt, "is", $days, $type);
} elseif ($baby_condition) {
    mysqli_stmt_bind_param($stmt, "is", $days, $baby);
} else {
...
<form method="POST">
    <center>
        ...
        for baby 
        <select name="baby">
            <option value="all" <?php echo (!isset($_POST['baby']) || $_POST['baby'] === "all") ? "selected" : ""; ?>>All</option>
            <?php
            foreach ($valid_babies as $b) {
                $sel = (isset($_POST['baby']) && $_POST['baby'] === $b) ? "selected" : "";
...
<table width="700" border="1" cellpadding="1" cellspacing="1" align="center">
    <tr>
        <th width="150px">Date</th>
        <th width="150px">Time</th>
        <th width="150px">Baby</th>
    ...
    while ($event = mysqli_fetch_assoc($results)) {
        ...
        $icon = $icons[$class] ?? "&#x2753;";
        $baby = htmlspecialchars($event['baby_name']);
        echo "<tr class='$class'>";
        echo "<td class='$class'>$date</td>";
        echo "<td class='$class'>$time</td>";
        echo "<td class='$class'>$baby</td>";
